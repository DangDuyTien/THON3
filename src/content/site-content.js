...
export function getSeasonalGalleryFocus(progress, horizontalTravel, photoCount = seasonalGallery.photos.length) {
  const trackCenters = photoCount === seasonalGalleryTrackCenters.length
    ? seasonalGalleryTrackCenters
    : Array.from({ length: photoCount }, (_, index) => (
      15.25 + (index * (185.5 - 15.25)) / Math.max(photoCount - 1, 1)
    ));
