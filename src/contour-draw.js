...
function updateSimulation(sim, clock, pointerX, pointerY) {
  ...
  // Update main seed nodes with smooth harmonic drift + pointer proximity interaction
  for (const n of sim.nodes) {
    ...
    // Soft cursor repulsion/attraction force
  ...
  // Dynamic satellite node spawner ("tạo thêm")
  ...
  // Virtual cursor field bump node
  activeNodes.push({
    isSatellite: false,
    life: 1.0,
    q: 0.7,
...
// Evaluate metaball potential field at (x, y)
...
// Marching Squares Isoline Extractor
...
// Midpoint Quadratic Bezier Interpolation
...
// Draw dynamic organic stream bridges between close nodes ("nối nhau")
...
export function createContourRenderer(canvas, quality = "standard") {
  ...
  function draw(time = 0) {
    ...
    // Draw multi-layered metaball isolines
    ...
    // Draw dynamic connecting stream bridges
    context.beginPath();
